...
const ReportesTab = ({ totalAccionistas: totalMockAcc = 10, pagos: pagosMock = [], accionistas: accionistasMock = [], anios = [] }) => {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:3000/api/accionistas')
      ...
      .catch(err => console.error("Error accionistas:", err));
  }, []);

  const currentPagos = usarDatosReales ? pagosReales : pagosMock;
  
  
  const totalAccCount = usarDatosReales ? accionistasReales.length : totalMockAcc;

  // Lógica de filtrado
  const obtenerPagosFiltrados = () => {
    return currentPagos.filter(p => {
      const fecha = p.fechaRegistro || p.fechaIngresoMulta;
      const anioMatch = filtros.anio === "Todos" || fecha?.includes(filtros.anio);
  ...
  const generarReporte = (tipo) => {
    ...
    setTimeout(() => {
      ...
      const nombreArchivo = `Reporte_${tipo.toUpperCase()}_ANEUPI_${usarDatosReales ? 'REAL' : 'MOCK'}`;
      ...
      if (tipo === 'accionistas') {
        const dataAcc = usarDatosReales ? accionistasReales : accionistasMock;
        autoTable(doc, {
          startY: 50,
          head: [['ID', 'Nombre', 'Código', 'Email', 'Estado']],
          body: dataAcc.map(a => [a.id, a.nombre, a.codigo, a.email, a.estado]),
          theme: 'striped',
        ...
        autoTable(doc, {
          ...
          body: filtrados.map(p => [
            p.id,
            p.accionista?.nombre || p.accionistaNombre || 'N/A',
    ...
    <div className="p-7">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8">
        ...
        <button
          ...
          className={`flex items-center gap-2 px-6 py-3 rounded-xl font-bold border transition-all shadow-md ${
            usarDatosReales ? 'bg-green-600 border-green-400 text-white' : 'bg-white text-aneupi-primary border-aneupi-primary/20'
          }`}
        >
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-xl shadow-lg p-6 border-2 border-aneupi-primary/20 mb-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setShowFilters(!showFilters)} className="px-4 py-2 bg-aneupi-primary text-white rounded-lg flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {tiposReporte.map(reporte => (
                <div key={reporte.id} className="bg-aneupi-bg-tertiary rounded-xl p-5 border-2 border-aneupi-primary/10 hover:border-aneupi-primary transition-all group">
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-2xl shadow-xl p-6 border-2 border-aneupi-primary/20 sticky top-4">
            <h3 className="text-xl font-bold text-aneupi-primary mb-6 flex items-center gap-2">
              <FaEye /> {usarDatosReales ? 'Vista Previa Real' : 'Vista Previa'}
            </h3>
            <div className="space-y-4">
              <div className="p-4 bg-aneupi-bg-tertiary rounded-xl border border-aneupi-primary/10">
                <p className="text-sm text-aneupi-text-muted mb-1 font-bold">Total Registros</p>
              ...
              <div className="p-4 bg-aneupi-bg-tertiary rounded-xl border border-aneupi-primary/10 flex items-center justify-between">
                <span className="text-sm text-aneupi-text-muted font-bold">Accionistas</span>
                <span className="text-xl font-black text-aneupi-primary flex items-center gap-2">
                  <FaDatabase className="text-sm" /> {totalAccCount}
                </span>
              </div>
            </div>
          </div>
